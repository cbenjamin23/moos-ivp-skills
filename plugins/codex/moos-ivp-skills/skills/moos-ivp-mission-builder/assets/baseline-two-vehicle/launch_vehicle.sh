...
VDEST_POS="80,-40"
SURVEY_POLYGON="0,0:80,0:80,-80:0,-80"
STOCK_SPD="1.4"
MAX_SPD="2.0"
...
for ARGI; do
  ...
  if [ "${ARGI}" = "--help" -o "${ARGI}" = "-h" ]; then
    ...
    echo "  --survey_poly=<poly>   Survey polygon           "
  ...
  elif [ "${ARGI:0:14}" = "--survey_poly=" ]; then
    SURVEY_POLYGON="${ARGI#--survey_poly=*}"
...
if [ "${VERBOSE}" = "yes" ]; then
  ...
  echo "SURVEY_POLY =   [${SURVEY_POLYGON}]"
fi
...
nsplug meta_vehicle.bhv targ_$VNAME.bhv $NSFLAGS \
  ...
  VDEST_POS=$VDEST_POS SURVEY_POLYGON=$SURVEY_POLYGON
